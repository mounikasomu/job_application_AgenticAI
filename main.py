...
def load_data() -> dict:
    """Load persisted JSON data adhering strictly to the system schema."""
    default_state = {
        "is_onboarded": False,
    ...
    try:
        with open(DATA_FILE, "r", encoding="utf-8") as f:
            ...
            for k, v in default_state.items():
                if k not in loaded:
                    loaded[k] = v
            # Sync is_onboarded boolean flag if current_profile_state exists
            if loaded.get("current_profile_state") and isinstance(loaded["current_profile_state"], dict) and loaded["current_profile_state"].get("name"):
                loaded["is_onboarded"] = True
            return loaded
...
async def save_profile(profile: dict):
    """Save the fully edited structured profile state and mark onboarded."""
    data = load_data()
    data["current_profile_state"] = profile
    data["is_onboarded"] = True
    save_data(data)
    return {"status": "success", "profile": profile, "is_onboarded": True}
...
async def get_applications():
    """Return all applications, onboarding status, and current profile state."""
    data = load_data()
    return {
        "is_onboarded": data.get("is_onboarded", False),
        "applications": data.get("applications", []),
        "current_profile_state": data.get("current_profile_state"),
        "profile_update_history": data.get("profile_update_history", data.get("profile_history", [])),
        "global_analysis": data.get("global_analysis", {}),
        "reference_resume_style": data.get("reference_resume_style"),
        "original_profile_text": data.get("original_profile_text"),
    }
...
async def analyze_rejection(app_id: str = Form(...)):
    ...
    if "profile_update_history" not in data:
        data["profile_update_history"] = []
    data["profile_update_history"].append(update_entry)
    data["profile_history"].append(update_entry)
...
async def get_global_analysis():
    ...
    if not saved:
        ...
        if len(rejected_apps) < 2:
            return {
                "has_data": False,
                "message": (
                    "Insufficient rejection data to generate cross-pattern insights. "
                    "Complete rejection analyses in Tab 3 to unlock your comprehensive SWOT report."
                ),
            }
        else:
            return {
