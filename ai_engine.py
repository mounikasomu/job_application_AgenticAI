...
def parse_candidate_profile(raw_text: str) -> Dict[str, Any]:
    ...
    for k in ["location", "github", "linkedin", "portfolio"]:
        if k not in credentials:
            credentials[k] = ""
    if "links" not in credentials or not isinstance(credentials["links"], list):
        links = []
        for lk in [credentials.get("github"), credentials.get("linkedin"), credentials.get("portfolio")]:
            if lk and lk.strip():
                links.append(lk.strip())
        credentials["links"] = links
